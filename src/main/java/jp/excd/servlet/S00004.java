	...
	private void composerResult(
		...
		try {
			...
			while (rs.next()) { //取得した作曲家情報をcrに纏める
				...
				cr.setJoinedDate(rs.getString("joined_date")); //登録日をcrへ
			...
			birthdayFormated = CommonUtils.profile(cr.birthday); //誕生日yyyy/mm/dd化
			
			String joinedDateFormated = CommonUtils.dateformat(cr.joinedDate); //登録日yyyy年mm月dd日化
			...
			while(rs.next()) {
				...
				sb.setImageFileName(rs.getString("image_file_name")); //画像名をsbへ
				...
				sb.setCutLenght(CommonUtils.cutLength(sb.getFormatHeight())); //画像をどれだけカットするか
				
				sb.setSongId(rs.getLong("id")); //曲ID
				
				String ratingTotalFormated =CommonUtils.valueformat(rs.getLong("rating_total")); //総感動指数フォーマット
				sb.setRatingTotalFormated(ratingTotalFormated); //総感動指数(フォーマット済み)をsbへ
				
				String ratingAverageFormated =CommonUtils.averageformat(rs.getDouble("rating_average")); //感動指数平均フォーマット
				sb.setRatingAverageFormated(ratingAverageFormated); //感動指数平均(フォーマット済み)をsbへ
				
				String totalListenCountFormated = CommonUtils.valueformat(rs.getLong("total_listen_count")); //再生回数フォーマット
				sb.setTotalListenCountFoemated(totalListenCountFormated); //再生回数(フォーマット済み)をsbへ
				
				String releaseDatetimeFormated = CommonUtils.epoch(rs.getDouble("release_datetime")); //公開日フォーマット
				sb.setReleaseDatetimeFormated(releaseDatetimeFormated); //公開日(フォーマット済み)をsbへ
			...
			//★出力する★
			composerBean.setUniqueCode(cr.uniqueCode); //ユニークコード
			composerBean.setNickname(cr.nickname); //ニックネーム
			composerBean.setMessage(cr.message); //メッセージ
			composerBean.setGenderResult(genderResult); //性別
			composerBean.setBirthdayFormated(birthdayFormated); //誕生日
			composerBean.setFbLink(cr.fbLink); //FaceBookリンク
			composerBean.setTwLink(cr.twLink); //Twitterリンク
